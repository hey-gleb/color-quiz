...
const GameScene: React.FC<GameSceneProps> = (props) => {
  ...
  const [guessOptions, setGuessOptions] = React.useState(
    generateColorOptions(colorToGuess),
  );
  ...
  const handleOptionClick = useCallback(
    (color: string) => {
      ...
      if (color === colorToGuess)
        newGameState.currentScore = newGameState.currentScore + 1;
